...
class Solution:
    ...
    def minDifficulty(self, jobDifficulty, d):
        ...
        n, dp = len(jobDifficulty), jobDifficulty[:]
        for i in range(1, n):
        ...
        for t in range(1, d):
            ...
            for i in range(t, n):
                m, j = dp[i - 1], i - 1
                while st[-1][0] < jobDifficulty[i]:
                    m, j = min(m, st[-1][1]), st[-1][2]
                    st.pop()
                if len(st) > 1:
                    temp.append(min(temp[j], m + jobDifficulty[i]))
                else:
                    temp.append(m + jobDifficulty[i])
                st.append((jobDifficulty[i], m, j))
            dp = temp
        return dp[-1]
# @lc code=end
