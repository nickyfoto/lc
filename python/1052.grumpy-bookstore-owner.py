...
class Solution:
    ...
    def maxSatisfied2(self, customers, grumpy, X):
        ...
        for i in range(n):
            x1 = customers[i:i+X]
            x2 = [customers[idx] for idx in list(range(i))+list(range(i+X, n)) if not grumpy[idx]]
            print(x1, x2)
            s = sum(x1) + sum(x2)
            if s > res:
                res = s
        return res
            # happy = sum(customers[i:i+X]) 
        
    def maxSatisfied(self, customers, grumpy, X):
        n = len(customers)
        res = - float('inf')
        i = 0
        h1 = sum(customers[i:i+X])
        h2 = sum([customers[idx] for idx in range(i+X,n) if not grumpy[idx]])
        res = - float('inf')
        while i+X <= n:
            if i > 0:
                h1 -= customers[i-1]
