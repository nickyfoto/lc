...
class Solution:
    ...
    def findRightInterval(self, intervals):
        ...
        s = sorted(zip([s[0] for s in intervals], range(n)))
        # print(s)
        res = []
        for i in range(n):
            pos = bisect.bisect_left(s, (intervals[i][1],))
            # print(pos)
            if pos == n:
                res.append(-1)
            else:
...
intervals = [ [1,2] ]
print(s.findRightInterval(intervals) == [-1])


intervals = [ [3,4], [2,3], [1,2] ]
print(s.findRightInterval(intervals) == [-1, 0, 1])


intervals = [ [1,4], [2,3], [3,4] ]
print(s.findRightInterval(intervals) == [-1, 2, -1])
